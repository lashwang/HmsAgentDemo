...
import com.huawei.android.hms.agent.opendevice.GetOaidApi;
import com.huawei.android.hms.agent.opendevice.GetOdidApi;
import com.huawei.android.hms.agent.opendevice.handler.GetOaidHandler;
...
public final class HMSAgent implements INoProguard {
    ...
    /**
     * 开放id接口封装 | OpenIdentifier interface Encapsulation
     * oaid 匿名设备标识，同一个设备上所有应用取得的值相同。用户可以主动重置或通过恢复出厂设置重置。
     * odid 开放给开发者的设备标识，同一设备并且同一个供应商不同应用获取标识是一样的，其他均不一样
     */
    public static final class OpenIdentifier {
        /**
         * 获取oaid接口 | Get Oaid method
         * @param handler
         */
        public static void getOaid(GetOaidHandler handler) {
            new GetOaidApi().getOaid(handler);
        }

        /**
         * 获取odid接口 | Get Odid method
         * @param handler
         */
        public static void getOdid(GetOdidHandler handler) {
            new GetOdidApi().getOdid(handler);
        }
    }
